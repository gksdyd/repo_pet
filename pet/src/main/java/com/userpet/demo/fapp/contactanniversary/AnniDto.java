package com.userpet.demo.fapp.contactanniversary;

public class AnniDto {

	private String anniSeq;
	private Integer anniCalendar;
	private String anniList;
	private String anniDate;
	private String contactAdd_contSeq;
	private Integer anniMain;
	private Integer deleNy;
	
	public String getAnniSeq() {
		return anniSeq;
	}
	public void setAnniSeq(String anniSeq) {
		this.anniSeq = anniSeq;
	}
	public Integer getAnniCalendar() {
		return anniCalendar;
	}
	public void setAnniCalendar(Integer anniCalendar) {
		this.anniCalendar = anniCalendar;
	}
	public String getAnniList() {
		return anniList;
	}
	public void setAnniList(String anniList) {
		this.anniList = anniList;
	}
	public String getAnniDate() {
		return anniDate;
	}
	public void setAnniDate(String anniDate) {
		this.anniDate = anniDate;
	}
	public String getContactAdd_contSeq() {
		return contactAdd_contSeq;
	}
	public void setContactAdd_contSeq(String contactAdd_contSeq) {
		this.contactAdd_contSeq = contactAdd_contSeq;
	}
	public Integer getAnniMain() {
		return anniMain;
	}
	public void setAnniMain(Integer anniMain) {
		this.anniMain = anniMain;
	}
	public Integer getDeleNy() {
		return deleNy;
	}
	public void setDeleNy(Integer deleNy) {
		this.deleNy = deleNy;
	}
}
